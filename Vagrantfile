...
SHELL
        when "centralRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            sudo bash -c 'echo "192.168.1.0/24 via 192.168.255.6 dev eth6" > /etc/sysconfig/network-scripts/route-eth6'
            sudo bash -c 'echo "192.168.2.0/24 via 192.168.255.10 dev eth5" > /etc/sysconfig/network-scripts/route-eth5'; sudo systemctl restart network
            sudo reboot
 SHELL
 ...
 SHELL
        when "office1Router"
          box.vm.provision "shell", run: "always", inline: <<-SKRIPT
            sudo bash -c 'echo "net.ipv4.conf.all.forwarding=1" >> /etc/sysctl.conf'; sudo sysctl -p
